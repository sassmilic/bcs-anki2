...
# --- Dictionary page parse (Gemini, text input from OCR library) ---

PARSE_DICTIONARY_PAGE_SYSTEM = (
    "You are a parser for raw OCR text from a Serbian-English thematic dictionary page. "
    "Each page lays out two numbered lists with the SAME numbers: first an English list, "
    "then a Serbian/BCS list with matching numbers. Your job is to pair them by number "
    "and return strict JSON. The OCR text may contain noise (stray characters, broken "
    "line breaks, mis-recognized punctuation) — be tolerant."
)

PARSE_DICTIONARY_PAGE_USER = """\
Below is the raw OCR text of a dictionary page. Parse it.

Return STRICT JSON only (no markdown, no code fences, no commentary), in this exact shape:

{{
  "section": "<the section title shown on the page, e.g. 'Geografija II'; empty string if none>",
  "entries": [
    {{"n": 1, "english": "<English term>", "serbian_raw": "<Serbian term as printed>"}},
    {{"n": 2, "english": "...", "serbian_raw": "..."}}
  ]
}}

Rules:
- Pair each numbered English entry with the Serbian entry that has the same number. The Serbian half restarts numbering from 1, so detect the boundary by where numbers reset.
- Multi-line entries: if a term wraps onto the next line(s), join it into one string with single spaces.
- Preserve parenthetical alternates and comma-separated forms verbatim in `serbian_raw` (e.g. "stena, hrid", "more (okean)"). Do NOT clean or normalize.
- Tolerate OCR noise: e.g. "l." or "I." at line start probably means "1.". Only emit an entry if you have BOTH a confident English and Serbian counterpart for that number.
- Skip any number where either side is missing or illegible — do not invent.
...
# --- Ijekavian normalization (Gemini batch) ---

IJEKAVIAN_NORMALIZE_SYSTEM = (
    "Ti si izvorni govornik bosanskog/hrvatskog/srpskog jezika i iskusni leksikograf. "
    "OBAVEZNO koristiš IJEKAVSKI izgovor i pravopis (rječnik, vrijeme, lijep, mlijeko, "
    "rijeka, stijena — NIKAD ekavski rečnik, vreme, lep, mleko, reka, stena). "
    "Tvoj zadatak je: za svaku zadanu riječ ili izraz, vratiti KANONSKI IJEKAVSKI oblik."
)

IJEKAVIAN_NORMALIZE_USER = """\
Za svaku stavku u JSON nizu ispod, vrati IJEKAVSKI oblik te riječi/izraza.

Pravila:
- Ako je ulaz već ijekavski, vrati ga nepromijenjen.
- Ako je ulaz ekavski (npr. "reka", "stena", "venac", "vetar"), vrati ijekavski oblik (npr. "rijeka", "stijena", "vijenac", "vjetar").
- Ako ulaz sadrži alternativne oblike odvojene zarezom (npr. "stena, hrid"), vrati SAMO prvi/glavni oblik u ijekavskoj formi.
- Ako ulaz sadrži zagradu sa alternativom (npr. "more (okean)"), vrati SAMO glavni oblik bez zagrade.
- Sačuvaj imenice u nominativu jednine, glagole u infinitivu, izraze u rječničkom obliku.
- Ako je ulaz višerječni izraz (npr. "naplavna ravnica"), zadrži ga kao izraz, samo u ijekavskoj formi.

Ulaz (JSON niz):
{words_json}

Izlaz: STRIKTNO JSON niz nizova istoga REDOSLIJEDA i DUŽINE kao ulaz, npr.:
["rijeka", "stijena", ...]

Vrati ISKLJUČIVO JSON. Bez objašnjenja, bez markdowna, bez prefiksa."""


# --- Simple "draw {word}" prompt for AI image fallback in dictionary pipeline ---
